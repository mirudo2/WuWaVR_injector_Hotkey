...
void Log(const std::string& message) {
    std::ofstream logFile("C:\\WutheringWavesVR\\loader_log.txt", std::ios_base::app);
    if (logFile.is_open()) {
...
extern "C" __declspec(dllexport) void LoadBackendDLL() {
    if (!dllLoaded.load()) {
        ...
        HMODULE hModule = LoadLibrary(L"C:\\WutheringWavesVR\\UEVRBackend.dll"); // Usando L prefix
        if (hModule != NULL) {
